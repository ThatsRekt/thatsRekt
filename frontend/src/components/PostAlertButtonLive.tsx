...
/**
 * PostAlertButtonLive — the full interactive "report" button with wagmi hooks.
 *
 * This module is lazily imported by PostAlertButton.tsx so that wagmi hook
 * imports (useAccount, useDisconnect, useIsWhitelisted, useConnect) remain
 * in the wagmi async chunk, NOT in the homepage-critical bundle.
 *
 ...
 */
export function PostAlertButtonLive({
  variant = 'desktop',
  onAfterClick,
}: {
 ...
 */
export function AccountChipLive() {
  const { address, isConnected } = useAccount()
