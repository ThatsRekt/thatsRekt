/**
 * AddressLabelCore — the pure rendering component for an address display.
 *
 * Zero wagmi/viem imports. Renders the address button + copy + explorer
 * link given a pre-computed `displayText` and the resolved `ensName`.
 *
 * Consumers: `AddressLabel` (sync base) and `AddressLabelEns` (wagmi-lazy).
 */
...
export interface AddressLabelCoreProps {
  addr: string
  chainSlug?: string
  full?: boolean
  /**
   * Show ENS primary name when present. Defaults true.
   * Set false when the literal hex is the point (e.g. /docs proxy table).
   */
  ens?: boolean
  /**
   * Pre-resolved ENS name to display, supplied by the lazy ENS enhancer
   * once the wagmi chunk is loaded. null = not yet resolved / no name.
   */
  ensName?: string | null
}

/**
 * Pure address label renderer. No side-effects, no network calls.
 * Accepts `ensName` from outside so that callers can control when / whether
 * the ENS name is shown.
 */
export function AddressLabelCore({
  addr,
  chainSlug,
  full = false,
  ens = true,
  ensName = null,
}: AddressLabelCoreProps) {
